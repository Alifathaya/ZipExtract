...
class FileBrowserViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun deleteSelected() {
        ...
        // Keep delete snappy: brief "Menghapus…" only — never kick off a full-library
        // "Mencari semua … di perangkat" scan in the progress overlay.
        activeJob?.cancel()
        activeJob = viewModelScope.launch {
            _uiState.update {
                it.copy(
                    progress = ProgressState(
                        title = "Menghapus…",
                        message = files.firstOrNull()?.name ?: "Menghapus file",
                        indeterminate = true,
                    ),
                    selectionMode = false,
                    selectedPaths = emptySet(),
